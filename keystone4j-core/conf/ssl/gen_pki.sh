...
organizationName        = InfinitiesSoft
organizationalUnitName  = Keystone4j
emailAddress            = pohsun@infinitiessoft.com
...
function generate_ca {
    ...
    openssl req -x509 -newkey rsa:2048 -days 21360 -out $CERTS_DIR/ca.pem \
...
function create_p12 {
    echo 'Issuing keystone.p12 Certificate ...'
    openssl pkcs12 -export -password pass:changeit -in $CERTS_DIR/ssl_cert.pem -inkey $PRIVATE_DIR/ssl_key.pem -out keystone4j.p12 -name "keystone4j" 
    check_error $?
}
